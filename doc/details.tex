...
\be
P(m_B^{\star} \vert \Omega, M_0, z, x_1, c, \sigma_{int} ) = \mathcal{N}(\mu(\Omega, z) - \alpha x_1 + \beta c + M_0, \sigma_{int}^2) 
\ee
...
&=& \int d\sigma_{int} \mathcal{N}(\mu(\Omega, z) - \alpha x_1 + \beta c + M_0, \sigma^{int}) P(t_0 \vert z, \Omega) P(x_1) P(c)\nonumber  
\eeqn

%mega) \\ &\times& P({x_1}_i) P(c_i) \int d\sigma_{int}
...
where 
\be
w = \mathcal{N}(\mu(\Omega, z) - \alpha {x_1}_i + \beta c_i + M_0, \sigma^{int}) / P(m_B^{\star})
\ee
