...
pgm.add_node(Node("zi", r"$z_i$", 4, 1, observed=True))

# True photometry
pgm.add_node(Node("fij", r"$f_{i,j}^{true}$", 4, 2, observed=False))

# Observed photometry
pgm.add_node(Node("fijo", r"$f_{i,j}$", 3, 1, observed=True))

# True time of peak
pgm.add_node(Node("t0true", r"$t_0^{\mathrm{true}}$", 5, 2))
...
pgm.add_node(Node("calibration", r'$\theta_c$', 1, 1))
...
pgm.add_edge('fij', 'fijo')
pgm.add_edge('calibration', 'fijo')
# Render and save.
pgm.render()
pgm.figure.savefig("snpgmwc.png", dpi=150)
